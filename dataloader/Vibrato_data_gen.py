...
if __name__ == "__main__":
    ...
    try:
        os.mkdir(os.path.join(config.DATASET_PATH, 'sawtooth_200'))
        # os.mkdir('sawtooth_200')
    except:
        pass

    try:
        os.mkdir(os.path.join(config.DATASET_PATH, 'sawtooth_200', 'songs'))
    ...
    for delta_f, smt, rate in itertools.product(delta_fs, semitones, rates):
        print(delta_f, rate)
        y = get_modulation(f0, delta_f, rate, len_sec)
        filename = os.path.join(config.DATASET_PATH, 'sawtooth_200', 'songs') + '/modulated_n_%d_%d_%d.wav' % ( int(f0), int(smt*100), int(rate*100))
        # filename = os.path.join(config.DATASET_PATH, 'sawtooth_200', 'songs') + '/modulated_%d_%d_%d.wav' % ( int(f0), int(delta_f), int(rate))
        # semitone, rate
        ...
        for F, BW, speech_name in zip(Fs, BWs, speech_names):
            ...
            filename = os.path.join(config.DATASET_PATH, 'sawtooth_200', 'songs') + '/modulated_%s_%d_%d_%d.wav' % ( 
                speech_name, int(f0), int(smt*100), int(rate*100))
            # filename = os.path.join(config.DATASET_PATH, 'sawtooth_200', 'songs') + '/modulated_%s_%d_%d_%d.wav' % ( 
            #     speech_name, int(f0), int(delta_f), int(rate))
            # librosa.output.write_wav(filename, y=y_modi, sr=SR)
            sf.write(filename, y_modi, SR, subtype='PCM_24')
